...
def train_and_plot_heatmap(name, classifier, param_grid):
    ...
    plt.tight_layout()
    plt.savefig(f'../result/images/mobile/{name}_heatmap.png')
    plt.show()
...
def plot_learning_curve(estimator, title, X_train, y_train, X_test, y_test, cv=None, n_jobs=None, train_sizes=np.linspace(0.1, 1.0, 5)):
    ...
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig(f'../result/images/mobile/{title}.png')
    return plt
...
# Selection of classifiers with optimal parameters
...
# Record test accuracy and training time for each model
...
plt.tight_layout()
plt.savefig('../result/images/mobile/model_accuracies.png')
plt.show()
...
plt.tight_layout()
plt.savefig('../result/images/mobile/model_training_times.png')
plt.show()
